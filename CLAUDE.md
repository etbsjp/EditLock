...
**現在は 1〜4 のすべてが 1.0.4 で一致している**（2026-09-20 実測で確認）。
`editlock.php` の `Version:` と `EDLK_VERSION`、`readme.txt` の `Stable tag:`、
`== Changelog ==` / `== Upgrade Notice ==` の `= 1.0.4 =` エントリ ―― 4箇所すべて 1.0.4。

★ 2026-09-20 訂正: ここには長く「1〜3 が 1.0.3 で一致し、4 には 1.0.4 のエントリが先に用意されている」と
書かれていたが、それは **1.0.4 をリリースする前の状態**の記述で、リリース後に更新されていなかった。
**版数運用の正本がこのファイルなので、ここが古いと次に触る人が確実に間違える。**
→ ★ **リリースのたびに、この行を実測値へ更新する。**
